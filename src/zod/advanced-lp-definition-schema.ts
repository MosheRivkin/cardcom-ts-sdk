...
export const advancedLpDefinitionSchema = z.object({
  ...
  ISOCoinName: z
    .string()
    .max(3)
    .describe('ISO name (alternative to IsoCoinId). ISO Coin Name from list : https://en.wikipedia.org/wiki/ISO_4217')
    .nullable()
    .nullish(),
  ...
  SelectedNumOfPayments: z
    .int()
    .default(0)
    .describe('Change the selectd number of payments in the UI selection box , default (MinNumOfPayments)')
    .nullable()
    .nullish(),
  ...
  Token: z
    .string()
    .describe('3-D secure and Token tranzactions can use token instead of card number,it will be used to charge the customer if exist ')
    .nullable()
    .nullish(),
  ...
  get ThreeDSecureState() {
    return threeDSecureStateSchema
      .describe(
        'should this Tranasaction pass 3DSecure? Auto (or null): 3-D Secure validation will be performed as it configured by CardCom definition. Enable - Transaction will pass 3-D Secure validation. Disable - Transaction will NOT pass 3-D Secure validation',
      )
      .nullable()
      .nullish()
  },
