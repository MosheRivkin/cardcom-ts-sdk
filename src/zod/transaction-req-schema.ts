...
export const transactionReqSchema = z.object({
  ...
  "ExternalUniqTranId": z.string().describe("External Uniq Tran Id , send your uniq trnasaction id to prevent duplication of transaction. if the same ExternalUniqTranId will be send you will receive and error code 608. see 'ExternalUniqUniqTranIdResponse' ").nullable().nullish(),
  "ExternalUniqUniqTranIdResponse": z.boolean().default(false).describe("External Uniq Tran Id , send your uniq trnasaction id to prevent duplication of transaction. if the same ExternalUniqTranId will be send you will receive and error code 608. see 'ExternalUniqUniqTranIdResponse' ").nullable().nullish(),
  ...
  "ISOCoinId": z.int().default(1).describe("ISO Coin Code ,1 - ILS, 2 - USD, else ISO Coin from list : https://en.wikipedia.org/wiki/ISO_4217").nullable().nullish(),
  get CustomFields(){
