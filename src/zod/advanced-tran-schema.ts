...
export const advancedTranSchema = z.object({
  ...
  "ISOCoinName": z.string().max(3).describe("ISO name (alternative and override to 'IsoCoinId'). ISO Coin Name from list : https://en.wikipedia.org/wiki/ISO_4217").nullable().nullish(),
  get JValidateType(){
                  return jValidateTypeSchema.describe("Type of transaction for credit card in operation : CreateTokenOnly or Suspended deal, can be J2 (simple card validation) or J5 (authoriz)").nullable().nullish()
                },
  ...
  "AccountIdToGetCardNumber": z.int().describe("Get the card number and expiretion date from account , if you need the account informatin for the invoice you need send the account Id in the document object : Document.AdvancedDefinition.AccountID and Document.AdvancedDefinition.IsLoadInfoFromAccountID=true").nullable().nullish(),
