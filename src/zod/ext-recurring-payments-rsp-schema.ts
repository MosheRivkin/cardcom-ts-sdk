 ...
 */
export const extRecurringPaymentsRspSchema = z
  .object({
    ...
    IsPrintNumOfPayments: z
      .boolean()
      .describe('add num of payments on the item descriptin in the document, add : (TotalNumOfBills of NumOfPaymentsAlreadyCharged )')
      .optional(),
    ...
    get ProdList() {
      return z
        .array(extProductRspSchema)
        .describe('list of prod to change , and amount will be taken for the system products table, only if IsByProducts==true ')
        .nullable()
        .nullish()
    },
  })
