...
function getLowProfileCreateUrl() {
  const res = { method: 'POST', url: `https://secure.cardcom.solutions/api/v11/LowProfile/Create` as const }
  return res
}
...
export async function lowProfileCreate(
  ...
  const res = await request<LowProfileCreateMutationResponse, ResponseErrorConfig<LowProfileCreate400 | LowProfileCreate401>, LowProfileCreateMutationRequest>({
    method: 'POST',
