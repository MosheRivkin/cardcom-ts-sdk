...
export async function companyOperationsAddDocumentToCompany(
  ...
  const requestData: { SupplierUserName: string; SupplierPassword: string; Secret: string; CompanyInternalID: string; DocumentsInfo?: { DocType: import("src/index").DocType; Name: string; Content: string; }[] | null | undefined; } | null = companyOperationsAddDocumentToCompanyMutationRequestSchema.parse(data)
